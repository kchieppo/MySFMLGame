...
PurpleRoom::PurpleRoom()
	:
	Room("Sprites/Environment/grass_tile.png", "PurpleRoom", 64, 64, false),
	roomMatrix{ {
...
void PurpleRoom::translateOut(Direction& dir)
{
	sf::Vector2f dirVec = getUnitVector(dir);
	sf::Vertex* quad;
	switch (dir)
...
void PurpleRoom::prepareTilePositions(Direction& transInDir)
{
	sf::Vertex* quad;
	for (int j = 0; j < roomDimRow; j++)
		for (int i = 0; i < roomDimCol; i++)
